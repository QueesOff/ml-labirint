...
function initializeLevel() {
    maze = (0, _mazeJs.generateMaze)(size);
    botX = 1;
    botY = 1;
    stack = [
        [
    ...
    (0, _mazeJs.drawMaze)(ctx, maze);
    (0, _mazeJs.drawBot)(ctx, botX, botY);
    statusDiv.textContent = `Level size: ${size}`;
    moveBot();
}
function moveBot() {
    (0, _mazeJs.drawMaze)(ctx, maze) // Перерисовываем лабиринт
    ;
    (0, _mazeJs.drawBot)(ctx, botX, botY) // Рисуем бота на новой позиции
    ;
    if (stack.length === 0) {
    ...
    for (let [newX, newY] of directions)if (newX > 0 && newX < size && newY > 0 && newY < size && maze[newY][newX] === 0) {
        stack.push([
            newX,
            newY
        ]);
...
},{"./maze.js":"3KMyC"}]},["farZc","8lqZg"], "8lqZg", "parcelRequire456c")
